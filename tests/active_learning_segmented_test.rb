...
class SegmentedActiveLearningTest < Minitest::Test
  def setup
  ...
  %w[scene7 scene8].each do |scene|
    define_method("test_external_scene_#{scene}") do
      ...
      setup_external_scene(scene_path, n:10, from_record: true)
  ...
  def setup_agentformer_config_instance
    ...
    @instance.instance_variable_set(:@segmented, '-segmented')
  end

  def setup_default_scene
    @instance.set_data_source(
      [TestAsset.get_path('steersim_binary_high_fps/sample1.bin')], [TestAsset.get_path('steersim_binary_high_fps/sample2.bin')], [])
